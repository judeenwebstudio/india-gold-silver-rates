...
export async function GET(request:Request,{params}:{params:Promise<{orderId:string}>}){
  ...
  }catch(error){const message=error instanceof Error?error.message:"";const forbidden=message.startsWith("ADMIN_")||message==="CSRF_REJECTED";return NextResponse.json({success:false,error:{message:forbidden?"Admin authorization required.":"Unable to generate document."}},{status:forbidden?403:500});}
}
