...
export async function GET() {
  try {
    const [products, rates] = await Promise.all([
      prisma.shopProduct.findMany({ where: { isActive: true }, orderBy: { metalType: 'asc' } }),
      getTrichyShopRates(),
    ]);
    return NextResponse.json({
      ...
      data: {
        ...
        products: products.map((product) => {
          ...
          return {
            id: product.id, slug: product.slug, name: product.name, metalType: product.metalType,
            purity: product.purity, description: product.description, imageUrl: product.imageUrl,
            weights, serviceChargePercent: product.serviceChargeBasisPoints / 100,
