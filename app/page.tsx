...
import { getCityDisplayRates, getMajorCityDisplayRates, getPublicLocations } from "@/lib/city-rate-service";

export const dynamic = "force-dynamic";

export default async function Home() {
  const locations = await getPublicLocations();
  const allCities = locations.flatMap((state) => state.cities.map((city) => ({ ...city, stateId: state.id })));
  const defaultCity = allCities.find((city) => city.slug === "chennai") ?? allCities[0];
  ...
  return (
    <>
      <Header />
      <main>
        <HomeRateExperience states={locations} initialSnapshot={initialSnapshot} />
        <HistoricalChart />
