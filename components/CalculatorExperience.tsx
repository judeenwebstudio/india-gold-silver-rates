...
export function CalculatorExperience({ states, initialSnapshot }: { states: PublicStateOption[]; initialSnapshot: PublicRateSnapshot }) {
  ...
  return <div className="space-y-8">
    <section className="rounded-3xl bg-stone-950 p-6 text-white sm:p-8"><p className="text-xs font-black uppercase tracking-[.18em] text-amber-300">Live city calculator</p><h1 className="mt-2 font-display text-4xl font-bold">Gold Rate Calculator &amp; Silver Value Calculator</h1><p className="mt-3 max-w-3xl text-stone-300">Select a city to load its indicative rates, then estimate gold value by purity and weight or calculate silver value for supported weights. Estimates exclude making charges and GST.</p><div className="mt-6"><LocationSelector states={states} selectedStateId={stateId} selectedCitySlug={citySlug} loading={loading} onStateChange={selectState} onCityChange={slug=>void selectCity(slug)}/></div>{error&&<p role="alert" className="mt-3 rounded-xl bg-red-950/60 p-3 text-red-100">{error}</p>}</section>
