...
type History = { records: Rate[]; availableDays: number; summary: null | { current: number; change: number; high: number; low: number } };
    ...
    <section id="historical" className="border-y border-stone-200 bg-[#f3efe7] py-14 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div><p className="eyebrow">Real stored data</p><h2 className="section-title mt-2">7-Day Movement</h2><p className="mt-3 max-w-2xl text-sm text-stone-600">Tiruchirappalli prices derived from stored IBJA publications and the configured city adjustment. Missing dates are never estimated.</p></div>
        ...
        <div className="mt-8 rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-7" aria-live="polite">
          {error ? <p className="py-20 text-center">{error}</p> : !data ? <p className="py-20 text-center">Loading stored rates…</p> : !points.length ? <p className="py-20 text-center">No stored rate history is available for this selection.</p> : <>
            {data.availableDays < 7 && <p className="mb-4 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-900">Showing {data.availableDays} available trading days; missing dates are not estimated.</p>}
            ...
            <p className="mt-4 text-xs text-stone-500">Source: {points.at(-1)!.sourceName} · Published {new Date(points.at(-1)!.sourcePublishedAt).toLocaleString("en-IN")} · Indicative calculated city rate</p>
          </>}
        </div>
      </div>
    </section>
