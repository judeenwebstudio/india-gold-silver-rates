...
export function HomeRateExperience({ states, initialSnapshot }: { states: PublicStateOption[]; initialSnapshot: PublicRateSnapshot }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <div className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
            <div><p className="text-xs font-black uppercase tracking-[0.22em] text-amber-300">Indicative city rate · {snapshot.location.cityName}</p><h1 className="mt-4 max-w-3xl font-display text-4xl font-bold leading-tight tracking-[-0.04em] sm:text-5xl lg:text-6xl">Gold Rate Today in India<span className="block text-amber-300">with clear silver rates.</span></h1><p className="mt-5 max-w-2xl text-base leading-7 text-stone-300 sm:text-lg">Check today&apos;s 22K and 24K gold rates and silver rate for {snapshot.location.cityName}, calculated from current national bullion rates with transparent city adjustments.</p><div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-stone-400"><span>Last updated {formatTimestamp(snapshot.lastUpdatedAt)} IST</span><span className="hidden h-4 w-px bg-white/15 sm:block" /><span>Source: {snapshot.source} · source time {formatTimestamp(snapshot.sourceTimestamp)} IST</span></div></div>
      ...
      <section id="rates" className="bg-[#fbfaf7] py-14 sm:py-20"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="eyebrow">{snapshot.location.cityName} market snapshot</p><h2 className="section-title mt-2">22K, 24K Gold &amp; Silver Rate Today in {snapshot.location.cityName}</h2></div><p className="max-w-lg text-sm leading-6 text-stone-500 sm:text-right">National base rate plus the configured city adjustment. Making charges and GST are not included.</p></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{snapshot.rates.map((rate, index) => <RateCard key={rate.id} rate={rate} featured={index === 0} />)}</div></div></section>

      <section id="calculator" className="bg-white py-14 sm:py-20"><div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:gap-16 lg:px-8"><div><p className="eyebrow">Plan your purchase</p><h2 className="section-title mt-2">Gold Rate Calculator</h2><p className="mt-4 max-w-xl text-base leading-7 text-stone-600">Estimate gold value by purity and weight using the currently selected indicative city rate. Making charges and GST are intentionally excluded.</p><Link href="/calculator" className="mt-5 inline-flex text-sm font-bold text-amber-800 underline decoration-amber-400 underline-offset-4 hover:text-amber-950">Open the full gold price calculator</Link></div><GoldCalculator rates={snapshot.rates} /></div></section>
