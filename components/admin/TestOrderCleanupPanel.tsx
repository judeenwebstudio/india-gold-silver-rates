...
export function TestOrderCleanupPanel({ orders }: { orders: CleanupOrder[] }) {
  ...
  const [selection,setSelection]=useState<{state:CleanupActionState;ids:string[]}>({state,ids:[]});
  const selected = selection.state === state ? selection.ids : [];
  const visibleIds = new Set(orders.map(order => order.id));
  const visibleSelected = selected.filter(id => visibleIds.has(id));
  const setSelected=(ids:string[])=>setSelection({state,ids});
  ...
  return <section className="mt-6 rounded-2xl border border-red-200 bg-red-50/60 p-5 shadow-sm">
    ...
    <p className="mt-2 max-w-4xl text-sm text-stone-700">Only PAYMENT_PENDING orders with no successful payment, finalized invoice, refund, shipment, pickup, or fulfilment history can be deleted. Failed payment attempts are allowed. Maximum 50 exact order IDs; wildcard deletion is not supported.</p>
    <form action={action} onSubmit={event=>{if(!window.confirm(`Permanently delete ${visibleSelected.length} selected unpaid orders? This cannot be undone.`)){event.preventDefault();setSelected([])}}} className="mt-4 space-y-4">
      {visibleSelected.map(id=><input key={id} type="hidden" name="cleanupOrderIds" value={id}/>)}
      ...
      <div className="flex flex-wrap gap-3">
        <button type="submit" disabled={pending||visibleSelected.length===0} className="rounded-xl bg-red-700 px-5 py-3 font-black text-white disabled:opacity-50">{pending?"Deleting…":"Permanently delete selected orders"}</button>
      </div>
    </form>
