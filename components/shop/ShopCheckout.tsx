...
type GstDetails={enabled:boolean;businessName:string;gstNumber:string;billingAddress:string;sameAsDelivery:boolean};
...
export function ShopCheckout() {
  ...
  const gstValid=!gst.enabled||(gst.businessName.trim().length>0&&gst.businessName.trim().length<=150&&effectiveBillingAddress.trim().length>0&&effectiveBillingAddress.trim().length<=500&&/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/.test(gst.gstNumber.trim().toUpperCase()));
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/v1/me/gst-profile",{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json()),
    ]).then(([shop, profile, saved, config,gstProfile]) => {
      ...
      if(gstProfile.data)setGst({enabled:false,businessName:gstProfile.data.businessName||"",gstNumber:gstProfile.data.gstNumber||"",billingAddress:gstProfile.data.billingAddress||"",sameAsDelivery:false});
  ...
  async function beginReview() {
    if (!gstValid) { setError("Complete all required GST billing fields and enter a valid 15-character GSTIN."); return; }
  ...
  async function pay() {
    if(gst.enabled&&!gstValid){setError("GST billing details are incomplete or invalid. Return to Edit Details and correct them.");return}
    ...
    try {
      ...
      const response = await fetch("/api/v1/shop/checkout", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ productId, weightGrams: weight, quantity, gateway, idempotencyKey, customer, gst:gst.enabled?{enabled:true,businessName:gst.businessName.trim(),gstNumber:gst.gstNumber.trim().toUpperCase(),billingAddress:effectiveBillingAddress.trim()}:{enabled:false}, ...(address.id ? { addressId: address.id } : { newAddress: address }) }) });
  ...
  return <div className="space-y-6"><div><p className="text-sm font-black uppercase text-amber-700">Secure checkout</p><h1 className="text-3xl font-black">{review ? "Review and pay" : "Customer and delivery details"}</h1><p className="text-stone-600">Customer Details · Delivery Address · Order Review · Payment</p></div>{error && <p role="alert" className="rounded-xl bg-red-50 p-4 font-semibold text-red-800">{error}</p>}
    <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]"><div className="space-y-6">
      ...
      <AddressSection addresses={addresses} selected={address} mode={addressMode} saveAddress={saveAddress} onSelect={selectAddress} onAddNew={()=>{setAddressMode("NEW");setAddress({...emptyAddress,fullName:customer.fullName,mobile:customer.mobile});}} onChange={setAddress} onSaveToggle={setSaveAddress} onUpdate={updateSavedAddress} onDelete={deleteSavedAddress} onDefault={setDefaultAddress}/><GstSection gst={gst} address={address} onChange={setGst}/><button onClick={beginReview} disabled={!valid} className="w-full rounded-xl bg-amber-500 p-4 font-black disabled:opacity-50">Review Order</button></>}
    ...
    </div><div className="space-y-4">{review&&gst.enabled&&<div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="font-black">GST Invoice</p><p>{gst.businessName}</p><p>{gst.gstNumber}</p><p className="whitespace-pre-line">{gst.billingAddress}</p></div>}<OrderSummary product={product} weight={weight} quantity={quantity} price={price}/></div></div>
  </div>;
}

function GstSection({gst,address,onChange}:{gst:GstDetails;address:Address;onChange:(value:GstDetails)=>void}){const delivery=`${address.addressLine1}${address.addressLine2?`, ${address.addressLine2}`:""}, ${address.city}, ${address.district}, ${address.state} - ${address.pincode}, India`;const gstError=gst.gstNumber&&!/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/.test(gst.gstNumber)?"Enter a valid 15-character GSTIN.":"";return <fieldset data-testid="gst-billing-section" className="rounded-2xl border border-amber-200 bg-white p-5"><legend className="px-2 text-lg font-black">3. GST Billing Details</legend><p className="font-bold">GST Invoice Required?</p><div className="mt-3 flex gap-6"><label><input type="radio" name="gstInvoiceRequired" value="NO" checked={!gst.enabled} onChange={()=>onChange({...gst,enabled:false})}/> No</label><label><input type="radio" name="gstInvoiceRequired" value="YES" checked={gst.enabled} onChange={()=>onChange({...gst,enabled:true})}/> Yes</label></div>{gst.enabled&&<div data-testid="gst-fields" className="mt-5 grid gap-4"><Field label="GST Registered Business Name" value={gst.businessName} onChange={businessName=>onChange({...gst,businessName:businessName.slice(0,150)})}/><Field label="GST Number" value={gst.gstNumber} onChange={gstNumber=>onChange({...gst,gstNumber:gstNumber.trim().toUpperCase().slice(0,15)})} error={gstError}/><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={gst.sameAsDelivery} onChange={event=>onChange({...gst,sameAsDelivery:event.target.checked,billingAddress:event.target.checked?delivery:gst.billingAddress})}/> Billing Address same as Delivery Address</label><label className="text-sm font-bold">GST Billing Address<textarea required maxLength={500} rows={4} value={gst.sameAsDelivery?delivery:gst.billingAddress} onChange={event=>onChange({...gst,billingAddress:event.target.value,sameAsDelivery:false})} className="mt-1 w-full rounded-xl border p-3"/></label></div>}</fieldset>}
