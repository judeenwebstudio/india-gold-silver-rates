...
export function ShopCheckout() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if(gstProfile.data)setGst({enabled:Boolean(gstProfile.data.isActive),businessName:gstProfile.data.businessName||"",gstNumber:gstProfile.data.gstNumber||"",billingAddress:gstProfile.data.billingAddress||"",sameAsDelivery:false});
...
function GstSection({gst,address,onChange}:{gst:GstDetails;address:Address;onChange:(value:GstDetails)=>void}){const delivery=`${address.addressLine1}${address.addressLine2?`, ${address.addressLine2}`:""}, ${address.city}, ${address.district}, ${address.state} - ${address.pincode}, India`;const gstError=gst.gstNumber&&!/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/.test(gst.gstNumber)?"Enter a valid 15-character GSTIN.":"";return <fieldset className="rounded-2xl border bg-white p-5"><legend className="px-2 text-lg font-black">Business Purchase (GST Invoice Required?)</legend><p className="font-bold">Do you have a GST Number?</p><div className="mt-3 flex gap-6"><label><input type="radio" checked={gst.enabled} onChange={()=>onChange({...gst,enabled:true})}/> Yes</label><label><input type="radio" checked={!gst.enabled} onChange={()=>onChange({...gst,enabled:false})}/> No</label></div>{gst.enabled&&<div className="mt-5 grid gap-4"><Field label="GST Registered Business Name" value={gst.businessName} onChange={businessName=>onChange({...gst,businessName:businessName.slice(0,150)})}/><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={gst.sameAsDelivery} onChange={event=>onChange({...gst,sameAsDelivery:event.target.checked,billingAddress:event.target.checked?delivery:gst.billingAddress})}/> Billing Address same as Delivery Address</label><label className="text-sm font-bold">GST Billing Address<textarea required maxLength={500} rows={4} value={gst.sameAsDelivery?delivery:gst.billingAddress} onChange={event=>onChange({...gst,billingAddress:event.target.value,sameAsDelivery:false})} className="mt-1 w-full rounded-xl border p-3"/></label><Field label="GST Number" value={gst.gstNumber} onChange={gstNumber=>onChange({...gst,gstNumber:gstNumber.trim().toUpperCase().slice(0,15)})} error={gstError}/></div>}</fieldset>}
