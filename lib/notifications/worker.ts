...
export async function processNotificationBatch(limit=25){
  ...
  for(const row of rows){
    const claimed=await prisma.notificationOutbox.updateMany({where:{id:row.id,status:row.status},data:{status:"PROCESSING",attemptCount:{increment:1}}});if(!claimed.count){skipped++;continue}
    try{
      ...
      if(result.disabled){await prisma.notificationOutbox.update({where:{id:row.id},data:{status:"PENDING",failureReason:"Provider disabled."}});disabled++;skipped++;continue}
