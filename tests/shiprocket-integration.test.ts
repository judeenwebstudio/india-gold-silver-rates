...
test("notification outbox retains shipment events",async()=>{const schema=await read("prisma/schema.prisma"),service=await read("lib/shiprocket/service.ts");assert.match(schema,/model NotificationOutbox/);assert.match(service,/notificationOutbox\.create/)});
